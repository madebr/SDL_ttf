...
class Releaser:
    ...
    def create_mingw_archives(self) -> None:
        ...
        for destdir, files in self.release_info["mingw"]["files"].items():
            assert destdir[0] == "/" and destdir[-1] == "/", f"'{destdir}' must begin and end with '/'"
            ...
            for file in files:
                ...
                logger.debug("Adding %s as %s", file, arcname)
                archived_files[arcname] = filepath
        print("... done")
