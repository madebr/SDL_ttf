...
"""
This script is shared between SDL2, SDL3, and all sattellite libraries.
...
class Releaser:
    ...
    def _configure_file(self, path: Path, extra_data: typing.Optional[dict[str, str]]=None) -> str:
        text = path.read_text()
        return self._configure_text(text)

    def _configure_text(self, text: str, extra_data: typing.Optional[dict[str, str]]=None) -> str:
        all_data = {
        ...
        for txt, repl in all_data.items():
            text = text.replace(f"@{txt}@", repl)
        remaining_matches = re.findall("@[A-Z_]{1,20}@", text)
        assert not remaining_matches, f"Failed to configure {repr(original_text)}. Remaining: {remaining_matches}"
        return text

    def _add_files_recursively(self, recorded_files: dict[str, Path], arc_dir: str, paths: list[Path]):
        logger.debug(f"_add_files_recursively({arc_dir=} {paths=})")
        for path in paths:
            arcname = self._arc_join(arc_dir, path.name)
            if path.is_file():
                logger.debug("Adding %s as %s", path, arcname)
                recorded_files[arcname] = path
            elif path.is_dir():
                self._add_files_recursively(recorded_files=recorded_files, arc_dir=self._arc_join(arc_dir, path.name), paths=list(path.iterdir()))
            else:
                raise ValueError(f"Unsupported file type to add recursively: {path}")

    @staticmethod
    def _arc_join(*parts: list[str]) -> str:
        assert all(p[:1] != "/" and p[-1:] != "/" for p in parts), f"None of {parts} may start or end with '/'"
        return "/".join(p for p in parts if p)

    def _record_files_tree(self, recorded_files: dict[str, Path], arc_dir: str, path: Path):
        for files_dir, _, files in os.walk(path):
            files_path = Path(files_dir)
            rel_files_path = files_path.relative_to(path)
            for f in files:
                recorded_files[self._arc_join(arc_dir, str(rel_files_path), f)] = files_path / f

    def _record_files_mapping(self, recorded_files: dict[str, Path], arc_dir: str, files_mapping: dict[str, list[str]], files_mapping_root: Path, extra_data: typing.Optional[dict[str, str]]=None):
        for rel_destdir, meta_file_globs in files_mapping.items():
            assert rel_destdir[:1] != "/" and rel_destdir[-1:] != "/", f"'{rel_destdir}' must not begin or end with '/'"
            assert "@" not in rel_destdir, f"'{rel_destdir}' should not contain a '@'"

            assert isinstance(meta_file_globs, list), f"'{repr(meta_file_globs)}' must be a list of paths/globs"
            for meta_file_glob in meta_file_globs:
                file_glob = self._configure_text(meta_file_glob, extra_data=extra_data)
                if ":" in file_glob:
                    original_path, new_filename = file_glob.split(":", 1)
                    ...
                    dest_arcpath = self._arc_join(arc_dir, rel_destdir, new_filename)
                    orig_path = files_mapping_root / original_path
                    logger.debug("Adding %s as %s", orig_path, dest_arcpath)
                    recorded_files[dest_arcpath] = orig_path
                else:
                    relative_file_paths = glob.glob(file_glob, root_dir=self.root)
                    assert relative_file_paths, f"Glob '{file_glob}' does not match any file"
                    self._add_files_recursively(recorded_files=recorded_files, arc_dir=self._arc_join(arc_dir, rel_destdir), paths=[files_mapping_root / p for p in relative_file_paths])

    def _store_recorded_files(self, archiver: Archiver, recorded_files: dict[str, Path]):
        for arcpath, path in recorded_files.items():
    ...
    def create_mingw_archives(self) -> None:
            ...
            for dep in self.release_info.get("dependencies", {}):
                ...
                with chdir(extract_path):
                    ...
                    for arch, triplet in ARCH_TO_TRIPLET.items():
                        ...
                        install_cmd = self._configure_text(install_cmd, extra_data=extra_configure_data)
        ...
        if "autotools" in self.release_info["mingw"]:
            for arch in self.release_info["mingw"]["autotools"]["archs"]:
                ...
                self._record_files_tree(recorded_files=recorded_files, arc_dir=self._arc_join(arc_root, triplet), path=install_path)
            ...
            for arch in self.release_info["mingw"]["cmake"]["archs"]:
                ...
                for arg_for_shared_static in args_for_shared_static:
                    ...
                    with self.section_printer.group(f"Build MinGW {triplet} (CMake)"):
                        self.executer.run(["cmake", "--build", str(build_path)], cwd=build_path, env=new_env)
        ...
        self._record_files_mapping(recorded_files=recorded_files, arc_dir=arc_root, files_mapping=self.release_info["mingw"]["files"], files_mapping_root=self.root)
        print("... done")
        ...
        with Archiver(zip_path=zip_path, tgz_path=tgz_path, txz_path=txz_path) as archiver:
            self._store_recorded_files(archiver=archiver, recorded_files=recorded_files)
        print("... done")
    ...
    def _archive_files_mapping(self, archiver: Archiver, arc_dir: str, files_mapping: dict[str, list[str]], files_mapping_root: Path, extra_data: typing.Optional[dict[str, str]]=None):
        for meta_arc_rel_destdir, file_rel_src_specs in files_mapping.items():
            arc_rel_destdir = self._configure_text(meta_arc_rel_destdir, extra_data=extra_data)
            for file_rel_src_spec in file_rel_src_specs:
                if ":" in file_rel_src_spec:
                    file_relpath, arc_name = file_rel_src_spec.rsplit(":", 1)
                    assert ":" not in file_relpath, f"':' can only be present once in {repr(file_rel_src_spec)}"
                    assert "/" not in arc_name, f"'/' cannot be present in destination {repr(arc_name)}"
                    path = files_mapping_root / file_relpath
                    arcpath = self._arc_join(arc_dir, arc_rel_destdir, arc_name)
                    if path.suffix == ".in":
                        data = self._configure_file(path, extra_data=extra_data)
                        logger.debug("Adding processed %s -> %s", path, arcpath)
                        archiver.add_file_data(arcpath=arcpath, data=data, mode=0o100644, time=datetime.datetime.now())
                    else:
                        logger.debug("Adding %s -> %s", path, arcpath)
                        archiver.add_file_path(arcpath=arcpath, path=path)
                else:
                    file_rel_src_spec = self._configure_text(file_rel_src_spec, extra_data=extra_data)
                    files = glob.glob(file_rel_src_spec, root_dir=files_mapping_root)
                    assert files, f"{repr(file_rel_src_spec)} must expand to at least one file"
                    for file in files:
                        path = files_mapping_root / file
                        arcpath = self._arc_join(arc_dir, arc_rel_destdir, path.name)
                        logger.debug("Adding %s -> %s", path, arcpath)
                        archiver.add_file_path(arcpath=arcpath, path=path)
    ...
    def create_android_archives(self, android_api: int, android_home: Path, android_ndk_home: Path) -> None:
        ...
        with Archiver(zip_path=aar_path) as archiver:
                ...
                with self.section_printer.group(f"Building for Android {android_api} {android_abi}"):
                    ...
                    for module_name, module_info in self.release_info["android"]["modules"].items():
                        if module_info["type"] == "library":
                            ...
                            arcdir_prefab_libs = f"prefab/modules/{module_name}/libs/android.{android_abi}"
                            archiver.add_file_path(arcpath=f"{arcdir_prefab_libs}/{library.name}", path=library)
                            archiver.add_file_data(arcpath=f"{arcdir_prefab_libs}/abi.json", data=self._get_prefab_abi_json_text(abi=android_abi, cpp=False, shared=library.suffix == ".so").encode(), time=datetime.datetime.now(), mode=0o100644)

                    if not java_jars_added and "jars" in self.release_info["android"]:
                        classes_jar_path = install_dir / self._configure_text(self.release_info["android"]["jars"]["classes"])
                        sources_jar_path = install_dir / self._configure_text(self.release_info["android"]["jars"]["sources"])
                        doc_jar_path = install_dir / self._configure_text(self.release_info["android"]["jars"]["doc"])
                        ...
                        archiver.add_file_path(arcpath="classes.jar", path=classes_jar_path)
                        archiver.add_file_path(arcpath="classes-sources.jar", path=sources_jar_path)
                        archiver.add_file_path(arcpath="classes-doc.jar", path=doc_jar_path)
            ...
            for module_name, module_info in self.release_info["android"]["modules"].items():
                ...
                archiver.add_file_data(arcpath=f"{arcdir_prefab_module}/module.json", data=self._get_prefab_module_json_text(library_name=library_name, export_libraries=export_libraries), mode=0o100644, time=datetime.datetime.now())
                arcdir_prefab_include = f"prefab/modules/{module_name}/include"
                if "includes" in module_info:
                    self._archive_files_mapping(archiver, arc_dir=arcdir_prefab_include, files_mapping=module_info["includes"], files_mapping_root=self.root)
                else:
                    archiver.add_file_data(arcpath=f"{arcdir_prefab_include}/.keep", data=b"\n", mode=0o100644, time=datetime.datetime.now())

            self._archive_files_mapping(archiver=archiver, arc_dir="", files_mapping=self.release_info["android"].get("files", {}), files_mapping_root=self.root)

            archiver.add_file_data(arcpath="prefab/prefab.json", data=self._get_prefab_json_text().encode(), mode=0o100644, time=datetime.datetime.now())
            archiver.add_file_data(arcpath="AndroidManifest.xml", data=self._get_android_manifest_text().encode(), mode=0o100644, time=datetime.datetime.now())
            archiver.add_git_hash(commit=self.commit, arcdir="")
    ...
    @staticmethod
    def _arch_to_vs_platform(arch: str) -> VsArchPlatformConfig:
        ARCH_TO_VS_PLATFORM = {
            "x86": VsArchPlatformConfig(arch="x86", platform="Win32", configuration="Release"),
            "x64": VsArchPlatformConfig(arch="x64", platform="x64", configuration="Release"),
            "arm64": VsArchPlatformConfig(arch="arm64", platform="ARM64", configuration="Release"),
        }
    ...
    def _build_msvc_msbuild(self, arch_platform: VsArchPlatformConfig, vs: VisualStudio):
        ...
        prebuilt_paths = set(self.root / full_prebuilt_path for prebuilt_path in self.release_info["msvc"]["msbuild"].get("prebuilt", []) for full_prebuilt_path in glob.glob(arch_platform.configure(prebuilt_path), root_dir=self.root))
        msbuild_paths = set(self.root / arch_platform.configure(f) for file_mapping in (self.release_info["msvc"]["msbuild"]["files-lib"], self.release_info["msvc"]["msbuild"]["files-devel"]) for files_list in file_mapping.values() for f in files_list)
        ...
        with Archiver(zip_path=zip_path) as archiver:
            arc_root = f""
            self._archive_files_mapping(archiver=archiver, arc_dir=arc_root, files_mapping=self.release_info["msvc"]["msbuild"]["files-lib"], files_mapping_root=self.root, extra_data=arch_platform.extra_data())
            self._archive_files_mapping(archiver=archiver, arc_dir=arc_root, files_mapping=self.release_info["msvc"]["files-lib"], files_mapping_root=self.root, extra_data=arch_platform.extra_data())
    ...
    def _build_msvc_cmake(self, arch_platform: VsArchPlatformConfig, dep_roots: list[Path]):
        ...
        built_paths = set(install_path / arch_platform.configure(f) for file_mapping in (self.release_info["msvc"]["cmake"]["files-lib"], self.release_info["msvc"]["cmake"]["files-devel"]) for files_list in file_mapping.values() for f in files_list)
        ...
        with Archiver(zip_path=zip_path) as archiver:
            arc_root = f""
            self._archive_files_mapping(archiver=archiver, arc_dir=arc_root, files_mapping=self.release_info["msvc"]["cmake"]["files-lib"], files_mapping_root=install_path, extra_data=arch_platform.extra_data())
            self._archive_files_mapping(archiver=archiver, arc_dir=arc_root, files_mapping=self.release_info["msvc"]["files-lib"], files_mapping_root=self.root, extra_data=arch_platform.extra_data())
        ...
        with Archiver(zip_path=zip_path) as archiver:
            if "msbuild" in self.release_info["msvc"]:
                for arch in self.release_info["msvc"]["msbuild"]["archs"]:
                    ...
                    self._archive_files_mapping(archiver=archiver, arc_dir=arc_root, files_mapping=self.release_info["msvc"]["msbuild"]["files-devel"], files_mapping_root=self.root, extra_data=arch_platform.extra_data())
            if "cmake" in self.release_info["msvc"]:
                for arch in self.release_info["msvc"]["cmake"]["archs"]:
                    arch_platform = self._arch_to_vs_platform(arch=arch)
                    self._archive_files_mapping(archiver=archiver, arc_dir=arc_root, files_mapping=self.release_info["msvc"]["cmake"]["files-devel"], files_mapping_root=self._arch_platform_to_install_path(arch_platform), extra_data=arch_platform.extra_data())
            self._archive_files_mapping(archiver=archiver, arc_dir=arc_root, files_mapping=self.release_info["msvc"]["files-devel"], files_mapping_root=self.root, extra_data=arch_platform.extra_data())
            archiver.add_git_hash(commit=self.commit, arcdir=arc_root)
